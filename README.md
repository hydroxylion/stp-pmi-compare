...
### 溯源标注：`(composite with <id>)`

复合公差的**后续段**，SFA 会在单元格末行标注它归属哪条复合公差：
...
这是**溯源元数据**（该段属于 486 号复合公差），不是 PMI 内容，语义表与图形 ta 表都可能带。
`strip_composite_marker()` 在 `normalize()` 与 `sfa_text_for()` 两处统一抹掉，理由有两条：

1. 括号里的数字会被 `_NUM` 当公差数值收进指纹 —— 一旦它落在取片段的那一行，
   就凭空多出一个数值，产生「开发侧数值缺失」这类假判；
2. 「SFA 有、开发侧未提取」的**反查行**走 `normalize()` 展示原文，不清洗会把这行
   溯源标注原样摊在结果表里，既噪声大又容易被误读成真值的一部分。

**只删这一种括号**：`[C]` / `<ST>` 是修饰符，`(point)` / `(line)` / `(area)` 是基准目标形式，
`(.281)` / `(⌀1.000)` 是参考尺寸 —— 都有语义，一律保留。全库扫描确认，
除 `(composite with N)` 外没有第二种同类的 SFA 溯源标注。
...
4. 查看三层指标、结果明细表、以及「🔧 提取缺陷清单」（带 SFA 原值，可单独导出 xlsx）。
...
缺陷清单是**按开发标注**聚合的（复合公差的两个子行算一条），列如下：

| 列 | 说明 |
|---|---|
| 分组 / 开发名称 / 开发标注 / Handle | 开发侧原文，交回开发侧时的定位信息 |
| 缺陷 | 缺陷码，多个用空格分隔 |
| **SFA条数** | 该标注对应几条 SFA 语义实体。`>1` 且带 `MRG` 就是「该拆没拆」 |
| **SFA语义文本** | SFA 语义表里的原值（逐字段）。复合公差给多条，用 `／` 分隔 |
| **SFA图形文本** | SFA 图形通道渲染的原文（图纸上实际显示的字） |
| 关联路径 / 详情 | 关联跳数与缺陷的完整说明 |

**为什么要带 SFA 原值**：清单原先只有开发侧文本，看到「NUM 数值缺失」还得回查报告
才知道缺的是哪个数、SFA 那边是多少。加上两个通道的原值后可以并排对照，
不用再开第二个窗口。

导出有两个入口，都是 xlsx：

- 缺陷清单展开区内的 **「📥 导出缺陷清单 (Excel)」** —— 只含缺陷，可直接转给开发侧；
- 页面底部的 **「📥 导出比对结果 (Excel)」** —— 三张表：`比对结果`（逐子行全字段）、
  `指标汇总`、`缺陷清单`（聚合口径）。

聚合口径与界面一致：缺陷按开发标注计数，复合公差的多个子行合并去重
（`MRG` 只在部分子行上判得出来，逐子行取第一条会漏）。
...
test_pmi_core.py       内核回归（286 项：指标口径、关联链路、缺陷检测（含 MRG 聚合与缺陷清单字段）、解析自检、列名驱动、ID 位数与基准通道、多视图复用与内容比对、实体名漂移、DMIA 通道、图形专用导出、文件对象入参、SFA 溯源标注）
test_ui_smoke.py       界面冒烟（54 项：AppTest 无头跑渲染分支 + 列口径 + 字段勾选开关 + 两个诊断面板 + 无真值态 + 缺陷清单 SFA 原值与两个导出入口）
