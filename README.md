...
属**匹配层**归因，不进缺陷层 —— 缺陷层是开发侧自检（不看 SFA），
「该不该拆」必须对照真值才能判断。
...
test_pmi_core.py       内核回归（268 项：指标口径、关联链路、缺陷检测、解析自检、列名驱动、ID 位数与基准通道、多视图复用与内容比对、实体名漂移、DMIA 通道、图形专用导出、文件对象入参）
...
| `nist_stc_09` | 18 个标注同时挂在 `MBD_A` + `MBD_A(Work)`；复合位置度开发侧合并成一条 | 三率 100%；差异 **15** 条（7 CNT + 8 复合公差）。旧代码因「复用即降级」会多出 11 条假差异（26 条） |
